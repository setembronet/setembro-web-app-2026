...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        if headings:
            ...
            # Enhanced to check for Hero components
            base_name = os.path.basename(filename).lower()
            if base_name in ['page.tsx', 'index.html', 'index.tsx']:
                 has_h1 = 'h1' in [h.lower() for h in headings]
                 has_hero = 'Hero' in content or 'PageHeader' in content
                 
                 if not has_h1 and not has_hero and has_long_text:
                     self.warnings.append(f"[Typography] {filename}: No h1 or Hero component found. Each page should have one primary heading.")
    ...
    def audit_directory(self, directory: str) -> None:
        extensions = {'.tsx', '.jsx', '.html', '.vue', '.svelte'}  # Removed .css
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'components', 'hooks', 'lib', 'utils', 'ui'}]
            for file in files:
