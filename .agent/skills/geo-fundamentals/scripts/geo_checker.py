...
SKIP_DIRS = {
    ...
    '__tests__', 'spec', 'docs', 'documentation', 'components', 'hooks', 'lib', 'utils', 'ui', 'store', 'context', 'constants', 'types'
}
...
def is_page_file(file_path: Path) -> bool:
    ...
    # Skip Next.js special files (non-content pages)
    if name in {'layout', 'template', 'loading', 'error', 'not-found', 'global-error', 'head', 'default'}:
        return False
...
def check_page(file_path: Path) -> dict:
    ...
    # 2. Heading Structure
    has_hero_h1 = 'Hero' in content or 'PageHeader' in content
    ...
    if h1_count == 1:
        ...
        if has_hero_h1:
             passed.append("Hero component detected (H1 proxy)")
        else:
    ...
    stat_patterns = [
        ...
        r'estudo\s+(mostra|revela)', # PT Research
        r'according to',            # Source attribution
        r'segundo',                 # PT Source
        r'data\s+(shows|reveals)',  # Data-backed claims
        r'dados\s+(mostram|revelam)', # PT Data
        r'\d+x\s+(faster|better|more)', # Comparison stats
        r'\d+x\s+(mais|melhor|rápido)', # PT Comparison
        r'(million|billion|trillion)', # Large numbers
        r'(milhão|milhões|bilhão|bilhões)', # PT Large numbers
    ]
...
def main():
    ...
    for result in results:
        status = "[OK]" if result['score'] >= 40 else "[!]"
