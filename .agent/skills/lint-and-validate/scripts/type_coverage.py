...
def check_typescript_coverage(project_path: Path) -> dict:
    """Check TypeScript type coverage using tsc if available."""
    issues = []
    passed = []
    stats = {'any_count': 0, 'untyped_functions': 0, 'total_functions': 0}
    
    # Try running tsc first as it is the authority
    ...
    if package_json.exists():
        try:
            ...
            if "type-check" in scripts:
                ...
                has_tsc = True
            elif "tsc" in pkg.get("devDependencies", {}) or "typescript" in pkg.get("dependencies", {}):
                cmd = ["npx", "tsc", "--noEmit"]
                has_tsc = True
            ...
            if has_tsc:
                ...
                if process.returncode == 0:
                    passed.append("[OK] TypeScript compilation passed (strict mode)")
                    return {'type': 'typescript', 'files': 1, 'passed': passed, 'issues': issues, 'stats': stats}
                else:
                    # Parse output for count of errors
                    error_count = process.stdout.count("error TS")
                    issues.append(f"[X] TypeScript compilation failed with {error_count} errors")
                    # We could return here, but maybe we want to fallback to regex? 
                    # No, if tsc fails, it fails.
                    return {'type': 'typescript', 'files': 1, 'passed': passed, 'issues': issues, 'stats': stats}

        except Exception as e:
            issues.append(f"[!] Failed to run tsc: {e}")

    # Fallback to Regex analysis (kept for backwards compatibility or non-npm projects)
    ...
    elif stats['any_count'] <= 10: # Increased threshold
    ...
    if stats['total_functions'] > 0:
        ...
        if typed_ratio >= 70: # Lowered threshold
